...
class PCBActiv(nn.Module):
    def __init__(self,in_channels,out_channels,batch_norm=True,step_name='',conv_bias=False,activation_name='relu'):
        super().__init__()
        if step_name == 'down_5':
            self.conv = PartialConv(in_channels,out_channels,5,2,2,bias=conv_bias)
        elif step_name == 'down_7':
            self.conv = PartialConv(in_channels,out_channels,7,2,3,bias=conv_bias)
        elif step_name == 'down_3':
            self.conv = PartialConv(in_channels,out_channels,3,2,1,bias=conv_bias)
        else:
            self.conv = PartialConv(in_channels,out_channels,3,1,1,bias=conv_bias)
